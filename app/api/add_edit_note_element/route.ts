  ...
  try {
    ...
    if (!parent_id) {
      return NextResponse.json(
        {
          data: "No 'parent_id' is specified!",
          status:
    ...
    // construct uri
    let uriStr = `http://${process.env.DATA_SERVER}:${process.env.DATA_SERVER_PORT}/api/v1/resources/add_edit_note_element?parent_id=${parent_id}&note=${note}`;
    if(note_id)
      uriStr += `&note_id=${note_id}`
